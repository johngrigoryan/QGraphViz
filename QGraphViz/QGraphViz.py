...
class QGraphViz_Core(QWidget):
    ...
    def __init__(
            ...
            # A callback called when a new connection is being added (should return True or False to accept or not the edge, as well as return the edge parameters)
            new_edge_created_callback=None,
            # A callback called when a new connection is created between two nodes using the GUI
            ...
            click_callback=None,  # A callback called when clicked on the part where is no any nodes or edges
    ...
    def mouseReleaseEvent(self, event):
        ...
        if n is not None:
            if self.node_selected_callback is not None:
                self.node_selected_callback(n, event)

        if e is not None:
            if self.edge_selected_callback is not None:
                self.edge_selected_callback(e, event)

        if n is None and e is None and s is None:
    ...
    def paintSubgraph(self, subgraph, painter, pen, brush):
        ...
        if ("fillcolor" in subgraph.kwargs.keys()):
            if (":" in subgraph.kwargs["fillcolor"]):
                gradient = QLinearGradient(subgraph.pos[0] - subgraph.size[0] / 2, subgraph.pos[1],
            ...
            if type(node) != Graph:
                ...
                if "fillcolor" in node.kwargs.keys():
                    if ":" in node.kwargs["fillcolor"]:
                        gradient = QLinearGradient(node.pos[0] - node.size[0] / 2, node.pos[1],
                ...
                if "shape" in node.kwargs.keys():
                    if node.kwargs["shape"] == "box":
                        ...
                        rect = QRect(int(gpos[0] - node.size[0] / 2), int(gpos[1] - 2 * node.size[1] / 3), node.size[0],
                                     node.size[1])
                        ...
                        rect = QRect(int(gpos[0] - node.size[0] / 2), int(gpos[1] - node.size[1] / 2), node.size[0],
                                     node.size[1])
                        ...
                        width = 0
                        height = 0
                        # if there is in the shape, the first part is the path, then width, then height
                        if "," in node.kwargs["shape"]:
...
class QGraphViz(QScrollArea):
    def __init__(self,
        ...
        self.core = QGraphViz_Core(
            ...
            # Callbacks
            new_edge_beingAdded_callback=new_edge_beingAdded_callback,
            # A callback called when a new connection is being added (should return True or False to accept or not the edge, as well as return the edge parameters)
            new_edge_created_callback=new_edge_created_callback,
            # A callback called when a new connection is created between two nodes using the GUI
            ...
            click_callback=click_callback,  # A callback called when clicked on the part where is no any nodes or edges
